...
test_that("ms_chart_combine assigns unique series idx/order across all charts", {
  ...
  idx   <- as.integer(xml2::xml_attr(xml2::xml_find_all(doc, "//c:ser/c:idx"),   "val"))
  order <- as.integer(xml2::xml_attr(xml2::xml_find_all(doc, "//c:ser/c:order"), "val"))
...
test_that("ms_chart_combine errors on column-name collision between charts", {
  ...
  expect_error(ms_chart_combine(bars = c1, line = c2),
               "column name")
})
...
test_that("ms_chart_combine errors on x-value mismatch", {
  ...
  expect_error(ms_chart_combine(bars = bars, target = line_bad_x),
               "x values")
})
